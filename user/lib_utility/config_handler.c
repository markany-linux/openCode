	...
	)
{
	...
	struct list_head *list_head = mild_null;
	pthread_mutex_t *list_lock = mild_null;
	...
	list_head = list__->list_head;
	list_lock = list__->list_lock;
	...
	list_add_tail( &config->ptr, list_head );
	...
	)
{
	...
	while( 0 == list_empty( list_head ) )
	{
		/// 3-1. 리스트의 현재 노드 포인터 획득
		config = list_entry( list_head->next, AGENT_CONFIG, ptr );
		/// 3-2. 리스트에서 현재 노드 제거
		list_del( &config->ptr );
		/// 3-3. 메모리 초기화
		memset( config, 0x00, sizeof( AGENT_CONFIG ) );
	...
	)
{
	...
	*list__ = mild_null;
	...
	list_head = ( struct list_head * )malloc( sizeof( *list_head ) );
	if( mild_null == list_head )
	{
		free( list );
		return mild_false;
	}

	list_lock = ( pthread_mutex_t * )malloc( sizeof( *list_lock ) );
	if( mild_null == list_lock )
	{
		free( list_head );
		free( list );
		return mild_false;
	...
	pthread_mutex_init( list_lock, mild_null );
