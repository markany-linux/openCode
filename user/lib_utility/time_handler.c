...
{
    get_time_readable( time__, buf__ );
}
...
{
    get_date_readable( time__, buf__ );
}
    ...
    )
{
    ...
    if( 0 == current__ )
    {
        set_time_zero( buf__, date__, time__ );
        return;
    }
...
{
    get_date_time_readable( current__, buf__, date__, time__ );
}
    ...
    )
{
    ...
    {
        /// 시작이면 획득 시간을 설정하고 반환
        app_time__->app_start = time;
        return;
    }

    /// 종료 시간 설정
    app_time__->app_end = time;

    /// 시간 차이 출력 함수 호출
    dprint_time_interval( app_time__, label__ );
}
